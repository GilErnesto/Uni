...
int ListSearch(List* l, const void* p) {
  ...
  while (node != NULL) {
    int cmp = l->compare(p, node->item);
    if (cmp == 0) {
      // Found! Move current to this position
      l->current = node;
      l->currentPos = pos;
      return 0;
    } else if (cmp < 0) {
      // Since list is sorted, element won't be found later
      break;
    }
    node = node->next;
    pos++;
...
void ListMove(List* l, int newPos) {
  ...
  if (newPos == -1) {  // move outside
    ...
    // Optimize: start from current position if it's closer
    if (l->currentPos >= 0 && abs(newPos - l->currentPos) < newPos) {
      node = l->current;
      pos = l->currentPos;
    } else {
      node = l->head;
    ...
    while (pos > newPos) {
      // Need to go backwards - restart from head
      node = l->head;
      pos = 0;
      while (pos < newPos) {
        node = node->next;
...
void* ListRemoveCurrent(List* l) {
  ...
  if (l->currentPos == 0)
    ...
    struct _ListNode* nodeToFree = l->current;
    l->current = l->current->next;  // Move current to next node
    
    free(nodeToFree);
    l->size--;
    
    // If we removed the last element in the middle, current might be NULL
    if (l->current == NULL) {
      l->currentPos = -1;
    }
    // currentPos stays the same since we move to the next element
  }
  return item;
}
