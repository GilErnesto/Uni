...
int DateIsValid(int yy, int mm, int dd) {
  return (DateMIN.year) <= yy && yy <= (DateMAX.year) && 1 <= mm && mm <= 12 && 1 <= dd && dd <= DateDaysInMonth(yy, mm);
}
...
void DateIncr(Date* d) {
  ...
  // Check if day exceeds the number of days in current month
...
void DateDecr(Date* d) {
  ...
  // Check if day becomes less than 1
  if (d->day < 1) {
    d->month--;
    
    // Check if month becomes less than 1
    if (d->month < 1) {
      d->month = 12;
      d->year--;
    }
    
    // Set day to the last day of the new current month
